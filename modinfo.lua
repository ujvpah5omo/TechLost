...
角色死亡可配置为丢失全部已学科技。每台科技站独立保存公开配方；由本 Mod 新锁定的配方可从风滚草和海盗猴获得蓝图。角色相关蓝图可配置为只按在线角色、已加入世界角色或全部已启用角色进入掉落池。技能树节点可配置为需要先学习蓝图许可，再按原版技能点和前置条件手动点亮；月后影后模式只控制掉落时间，月亮裂隙和暗影裂隙都开启后才掉落技能节点蓝图。开启技能树节点蓝图后，海盗猴首领大副必掉击杀者角色的技能节点蓝图，仍受裂隙掉落条件限制。本 Mod 蓝图池生成的地面蓝图可配置为经过数次下雨后消失。
]]
author = "Codex"
version = "2.10.9"
...
configuration_options =
{
    ...
    },
    {
        ...
        label = "技能树节点蓝图",
        hover = "开启后，技能树节点需要先学习蓝图许可，再消耗技能点并满足前置条件手动点亮；月后影后模式只控制掉落时间，双裂隙开启后才掉落技能节点蓝图。海盗猴首领大副必掉击杀者角色节点蓝图，仍受裂隙掉落条件限制。",
        options =
        {
            { description = "关闭", data = false },
