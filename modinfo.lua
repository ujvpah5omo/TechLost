...
角色死亡可配置为丢失全部已学科技。每台科技站独立保存公开配方；由本 Mod 新锁定的配方可从风滚草和海盗猴获得蓝图。角色相关蓝图可配置为只按在线角色、已加入世界角色或全部已启用角色进入掉落池。技能树节点可配置为需要先学习蓝图许可，再按原版技能点和前置条件手动点亮；进入世界后会取消未获许可的受控技能。本 Mod 蓝图池生成的地面蓝图可配置为经过数次下雨后消失。
]]
author = "Codex"
version = "2.10.6"
...
configuration_options =
{
    ...
    },
    {
        ...
        label = "技能树节点蓝图",
        hover = "开启后，风滚草和海盗猴可额外掉落技能树节点蓝图；角色范围由“角色蓝图池过滤”控制。对应角色学习蓝图后获得点亮许可，仍需自己消耗技能点并满足前置条件；进世界会取消未获许可的受控技能。",
        options =
        {
            { description = "关闭", data = false },
            { description = "开启", data = true },
